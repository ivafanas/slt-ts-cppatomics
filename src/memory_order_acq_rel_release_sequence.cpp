...
template <typename T> bool test(int num_attempts) {
...
int main(int argc, const char **argv) {
  ...
  bool succeed = true;
  succeed &= test<std::uint8_t>(num_attempts);
  succeed &= test<std::uint16_t>(num_attempts);
  succeed &= test<std::uint32_t>(num_attempts);
  succeed &= test<std::uint64_t>(num_attempts);
